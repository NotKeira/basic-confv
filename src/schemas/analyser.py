...
from ..types import SchemaName, SchemaNameMap
...
class SchemaAnalyser:
    ...
    @staticmethod
    def analyse_schema(file_path: str) -> SchemaName:
        ...
        schema_map: SchemaNameMap = {"nginx.conf": "nginx", "package.json": "package", "docker-compose.yml": "docker",
        ...
        if filename in schema_map:
            schema_name: SchemaName = schema_map[filename]
            print(f"✅ Detected schema: {schema_name}")
            return schema_name

        # Extension-based fallback
        if extension in schema_map:
            schema_name: SchemaName = schema_map[extension]
